class Player {
    ...
    movimento(moveset: string, delta: number) {
    ...
    async update(obj: Player) {
        ...
        if (await this.colisao.colidiu(obj.colisao)) {
        ...
        if (await this.ataque.colidiu(obj.colisao) && !obj.isDefense) {
        ...
        } else if(await this.ataque.colidiu(obj.colisao) && obj.isDefense) {
    ...
    setDefesa(valor:number) {

        this.defesa = valor

    }

    levarDano(dano: number) {
    ...
    async render(obj: Colisao){
        ...
        if (debug) {
            ...
            await this.colisao.render(obj)
            await this.ataque.render(obj)
            ctx.font = "20px arial"
